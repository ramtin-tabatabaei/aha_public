"""
Run an RLBench task without injected failures and summarize torque telemetry.

Example:
  python detectors/torque_stats/run.py --task basketball_in_hoop --episodes 3
"""
...
COLLISION_DETECTOR_PATH = str(paths.SOURCE_DIR / 'detectors' / 'collision')
CONFIGS_PATH = os.path.join(LOCAL_FAILGEN_PATH, "failgen/configs")
DEFAULT_COPPELIASIM_ROOT = str(paths.COPPELIASIM_ROOT)

for path in (COLLISION_DETECTOR_PATH, LOCAL_FAILGEN_PATH, REPO_ROOT):
    if path not in sys.path:
        sys.path.insert(0, path)


DEFAULT_SAVE_DIR = str(paths.OUTPUT_DIR / 'torque_stats')
ENV_READY_FLAG = "AHA_TORQUE_STATS_ENV_READY"
METRICS = ("torque_norm", "torque_delta")
...
def print_stats(title, norm_stats, delta_stats):
    print(f"\n{title}")
    print("-" * len(title))
    print(
        f"{'metric':<14} {'count':>8} {'mean':>12} {'std':>12} "
        f"{'min':>12} {'max':>12}"
    )
    for label, stats in (("torque_norm", norm_stats), ("torque_delta", delta_stats)):
        print(
            f"{label:<14} {stats['count']:>8d} "
...
def stats_rows(task_name, scope, norm_stats, delta_stats):
    rows = []
    for metric, stats in (
        ("torque_norm", norm_stats),
        ("torque_delta", delta_stats),
    ):
...
def task_stats_json(task_name, episodes_requested, summary_rows):
    metric_data = {}
    for metric in METRICS:
        ...
        metric_data[metric] = {
            ...
            "aggregate_all_frames": aggregate,
        }
...
def write_task_stats_json(save_dir, task_name, episodes_requested, summary_rows):
    data = task_stats_json(task_name, episodes_requested, summary_rows)
    path = save_dir / f"{task_name}_success_torque_stats.json"
    write_json(path, data)
    return path


def ensure_task_stats_json(save_dir, task_name, episodes_requested):
    json_path = save_dir / f"{task_name}_success_torque_stats.json"
    if json_path.exists():
        return json_path
    return None
...
def collect_task_rows(args, task_name, save_dir, obs_to_row, update_deltas, env_cls):
    ...
    try:
        for episode in range(args.episodes):
            ...
            norm_values = [row["torque_norm"] for row in logs]
            delta_values = [row["torque_delta"] for row in logs]
            norm_stats = summarize(norm_values)
            delta_stats = summarize(delta_values)
            print_stats(f"Episode {episode} stats", norm_stats, delta_stats)

            summary.extend(
                stats_rows(task_name, f"episode_{episode}", norm_stats, delta_stats)
            )
            episode_norm_stats.append(norm_stats)
            episode_delta_stats.append(delta_stats)
            all_rows.extend(logs)

        if all_rows:
            mean_episode_norm = mean_of_episode_stats(episode_norm_stats)
            mean_episode_delta = mean_of_episode_stats(episode_delta_stats)
            print_stats(
                f"{task_name} mean of episode stats",
                mean_episode_norm,
                mean_episode_delta,
            )
            summary.extend(
            ...
            aggregate_norm = summarize([row["torque_norm"] for row in all_rows])
            aggregate_delta = summarize([row["torque_delta"] for row in all_rows])
            print_stats(
                f"{task_name} aggregate stats", aggregate_norm, aggregate_delta
            )
            summary.extend(
                stats_rows(task_name, "aggregate", aggregate_norm, aggregate_delta)
            )

            json_path = write_task_stats_json(
                save_dir, task_name, args.episodes, summary
            )
...
def all_tasks_stats_json(save_dir, task_names, episodes_requested, failed_tasks):
    ...
    for task_name in task_names:
        ensure_task_stats_json(save_dir, task_name, episodes_requested)
        data = read_task_stats_json(
            save_dir / f"{task_name}_success_torque_stats.json"
        )
        if data is None:
            continue
        tasks.append(data)
    ...
    for metric in METRICS:
        ...
        if task_metric_stats:
            aggregate_stats = [
                task.get("metrics", {}).get(metric, {}).get("aggregate_all_frames")
                for task in tasks
            ]
            metrics[metric] = {
                ...
                "aggregate_all_frames": combine_aggregate_stats(aggregate_stats),
            }
        else:
            metrics[metric] = {
                "average_of_task_episode_averages": None,
                "aggregate_all_frames": None,
            }
...
def launch_parallel_task_batch(args, task_names, save_dir):
    ...
    def start_task(task_name):
        ...
        cmd = [
            sys.executable,
            str(paths.SOURCE_DIR / 'detectors/torque_stats/run.py'),
            "--task",
            task_name,
            "--episodes",
            str(args.episodes),
...
def run(args):
    ...
    if workers > 1 and len(task_names) > 1:
        ...
        if completed_tasks:
            ...
            all_json_path = save_dir / "ALL_TASKS_success_torque_stats.json"
            write_json(
                all_json_path,
        ...
        for task_index, task_name in enumerate(task_names, start=1):
            ...
            try:
                task_rows, task_summary = collect_task_rows(
                    args,
                    task_name,
                    save_dir,
                    obs_to_row,
